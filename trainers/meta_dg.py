...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label_idx = None, ctx = None, vis_ctx = None):
        ...
        reg_image = F.l1_loss(image_features, image_features_zs.cuda(),
                                    reduction='mean') * 10
        if self.training:
            return logits, reg_text, reg_image
        else:
            return logits
...
class  LSLRGradientDescentLearningRule(nn.Module):
    ...
    def update_params(self, ctx, grads_wrt_params_a, grads_wrt_params_b, generated_con, generated_alpha, generated_beta, lr, num_step):
        ...
        new_ctx = [] 
        for i in range(self.layers):
            new_ctx.append((1 - generated_con[i] * self.con_list[i][num_step] * (lr/self.lr) ) * ctx[i,:,:] \
                            - generated_alpha[i] * self.alpha_list[i][num_step] * (lr/self.lr) * grads_wrt_params_a[i,:,:] \
                            - generated_beta[i] * self.beta_list[i][num_step] * (lr/self.lr) * grads_wrt_params_b[i,:,:] )
        new_ctx = torch.stack(new_ctx)
        return new_ctx
   

class MAMLFewShotClassifier(nn.Module):
    def __init__(self, cfg, model, classnames, device, num_layers, meta_step, init_lr):
        """
        ...
        self.inner_loop_optimizer = LSLRGradientDescentLearningRule(total_num_inner_loop_steps=cfg.TRAINER.META.META_STEP, layers=num_layers, lr=init_lr)
    ...
    def forward(self, image, label, domain, lr):
        dd = domain.tolist()
        N = len(set(dd))
        ...
        for i in range(N):
            ...
            for k in range(self.meta_step):
                ...
                logits_ce_support, reg_text, reg_image = self.model(image_train[i], None, ctx, vis_ctx)
                support_loss = F.cross_entropy(logits_ce_support, label_train[i])

                grads_wrt_ctx = autograd.grad(support_loss, ctx, retain_graph=True)[0]
                reg_wrt_ctx = autograd.grad(reg_text, ctx, retain_graph=True)[0]
                ...
                generated_con, generated_alpha, generated_beta = torch.split(generated_params, split_size_or_sections=self.layers)

                ctx = self.inner_loop_optimizer.update_params(ctx=ctx,
                                            grads_wrt_params_a=grads_wrt_ctx,
                                            grads_wrt_params_b=reg_wrt_ctx,
                                            generated_con=generated_con,
                                            generated_alpha=generated_alpha,
                                            generated_beta=generated_beta,
                                            lr = lr,
                                            num_step=k)

                grads_wrt_vis = autograd.grad(support_loss, vis_ctx, retain_graph=True)[0]
                ...
                if k == self.meta_step - 1:
                    logits_ce_query, reg_image, reg_text = self.model(image_test[i], None, ctx, vis_ctx)
                    query_loss = F.cross_entropy(logits_ce_query, label_test[i])
...
class Meta_DG(TrainerX):
    ...
    def build_model(self):
        ...
        self.meta_init_lr = cfg.OPTIM_META.LR
        self.min_lr = cfg.OPTIM_META.WARMUP_MIN_LR

        print("Building custom CLIP")
        print(self.num_layers)
        self.model = CustomCLIP(cfg, classnames, clip_model)
        self.meta_model = MAMLFewShotClassifier(cfg, self.model, classnames, self.device, self.num_layers, self.meta_step, self.meta_init_lr)
    ...
    def forward_backward(self, batch):
        ...
        logits_ce, reg_text, reg_image = self.model(image)
        loss_ce = F.cross_entropy(logits_ce, label)
        self.model_backward_and_update(loss_ce)

        meta_loss = self.meta_model(image, label, domain, lr=self.get_current_lr())
