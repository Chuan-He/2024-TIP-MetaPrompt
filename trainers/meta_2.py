...
class  LSLRGradientDescentLearningRule(nn.Module):
    ...
    def __init__(self, device, total_num_inner_loop_steps, layers, use_learnable_learning_rates=True, use_learnable_weight_decay=True):
        ...
        for i in range(layers):
            ...
            #     requires_grad=self.use_learnable_weight_decay))
    ...
    def update_params_t(self, ctx, support_grads_mix, support_grads_t, lr, num_step):
        ...
        # for i in range(self.layers):
        #      per_step_task_embedding.append(ctx[i,:,:].mean())
        for i in range(self.layers):
        ...
        new_ctx = [] 
        for i in range(self.layers):
            new_ctx.append(ctx[i,:,:] - generated_beta[i] * self.beta_list[i][num_step] * lr * grads_wrt_params_b[i,:,:] - \
                                             generated_alpha[i] * self.alpha_list[i][num_step] * lr * grads_wrt_params_a[i,:,:])
        new_ctx = torch.stack(new_ctx)
        return new_ctx

    def update_params_i(self, vis_ctx, support_grads_mix, support_grads_i, lr, num_step):
        """Applies a single gradient descent update to all parameters.
        All parameter updates are performed using in-place operations and so
        nothing is returned.
        Args:
        ...
        per_step_task_embedding = []
        # for i in range(self.layers):
        #      per_step_task_embedding.append(vis_ctx[i,:,:].mean())
        for i in range(self.layers):
            per_step_task_embedding.append(support_grads_mix[i,:,:].mean())
        for i in range(self.layers):
        ...
        new_vis_ctx = [] 
        for i in range(self.layers):
            # new_vis_ctx.append(vis_ctx[i,:,:] - generated_beta[i,:,:] * self.alpha_list[i][num_step] * grads_wrt_params_b[i,:,:] - \
            #                                 generated_alpha[i,:,:] * self.beta_list[i][num_step] * grads_wrt_params_a[i,:,:])
            new_vis_ctx.append(vis_ctx[i,:,:] - generated_beta[i] * self.beta_list[i][num_step] * lr * grads_wrt_params_b[i,:,:] - \
                                             generated_alpha[i] * self.alpha_list[i][num_step] * lr * grads_wrt_params_a[i,:,:])
    ...
    def forward_backward(self, batch):
        ...
        for i in range(N):
            ...
            for k in range(self.meta_step):
                ...
                support_grads_mix = autograd.grad(support_loss_mix, ctx)[0]
                support_grads_t = autograd.grad(support_loss_t, ctx)[0]

                new_ctx = self.meta_model.update_params_t(ctx=ctx,
                                            support_grads_mix=support_grads_mix,
                                            support_grads_t=support_grads_t,
                                            lr = self.get_current_lr(),
                                            num_step=k)

                support_grads_mix = autograd.grad(support_loss_mix, vis_ctx)[0]
                support_grads_i = autograd.grad(support_loss_i, vis_ctx)[0]

                new_vis_ctx = self.meta_model.update_params_i(vis_ctx=vis_ctx,
                                            support_grads_mix=support_grads_mix,
                                            support_grads_i=support_grads_i,
                                            lr = self.get_current_lr(),
                                            num_step=k)
                ctx = new_ctx
                vis_ctx = new_vis_ctx
