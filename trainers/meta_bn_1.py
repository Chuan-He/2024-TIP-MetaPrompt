...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label_idx = None, ctx = None, vis_ctx = None, step = None):
        ...
        if self.training:
            ...
            return logits_ce, score_image, score_text
        else:
            return logits_ce
    ...
    def forward(self, image, label, lr):
        ...
        for i in range(N):
            ...
            for k in range(self.meta_step):
                ...
                logits_ce_sup, reg_image, reg_text = self.model(image, cls, ctx, vis_ctx, k)
                ...
                if k == self.meta_step - 1:
                    logits_ce_query = self.model(image_li[i], cls_li[i], ctx, vis_ctx)
