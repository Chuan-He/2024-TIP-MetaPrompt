...
class CustomCLIP(nn.Module):
    ...
    def forward_mix(self, image, label, image_sup, label_sup, mix_ids, lam, ctx, vis_ctx):
        tokenized_prompts = self.tokenized_prompts
        logit_scale = self.logit_scale.exp()
...
class  LSLRGradientDescentLearningRule(nn.Module):
    ...
    def __init__(self, device, total_num_inner_loop_steps, layers, lr):
...
class MAMLFewShotClassifier(nn.Module):
    def __init__(self, cfg, model, classnames, device, num_layers, meta_step, meta_lr):
        """
        ...
        self.inner_loop_optimizer = LSLRGradientDescentLearningRule(device=device, total_num_inner_loop_steps=cfg.TRAINER.META.META_STEP, layers=num_layers, lr=meta_lr)
    ...
    def forward(self, image, label, unique_label, lr):
        ...
        for _ in range(1):
            ...
            for k in range(self.meta_step):
                ...
                logits_ce_support, reg_image, reg_text = self.model(x_sup, y_sup, ctx, vis_ctx)
                support_loss_mix = F.cross_entropy(logits_ce_support, y_sup)
                ...
                if k == self.meta_step - 1:
                    logits_ce_query, reg_image, reg_text = self.model(x_qry, y_qry, ctx, vis_ctx)
                    query_loss = F.cross_entropy(logits_ce_query, y_qry)
...
class Meta_B2N(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if len(unique_label) != 1:
            meta_loss = self.meta_model(image, label, unique_label, lr=self.get_current_lr())
