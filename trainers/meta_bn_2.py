...
class VisionEncoder(nn.Module):
    ...
    def forward(self, x, ctx_v):
        ...
        for i in range(self.layers):
            if 1 <= i < self.layer_p:
                prefix = x[:-self.n_pro, :, :]
                x = torch.cat([prefix, ctx_v[i].permute(1, 0, 2)], dim=0)
...
class VisionPromptLearner(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.ctx = nn.ParameterList([nn.Parameter(torch.empty(n_pro, ctx_dim, dtype=self.dtype)) for _ in range(self.layer_p)])
        for single_para in self.ctx:
            nn.init.normal_(single_para, std=0.02)

        
    def forward(self, x, ctx_v=None):
        batch_size = x.shape[0]
        if ctx_v == None:
            ctx_v = self.ctx

        new_ctx_v = []
        for i in range(len(ctx_v)):
            new_ctx_v.append(ctx_v[i].unsqueeze(0).expand(batch_size, -1, -1))
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, ctx):
        ...
        for i in range(self.layers):
            if 1 <= i < self.layer_p:
                ...
                x = torch.cat([prefix, ctx[i-1].permute(1, 0, 2), suffix], dim=0) 
            x = self.transformer[i](x)
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.ctx0 = nn.Parameter(ctx_vectors) # parameters of text prompt to be learned
        # ctx_vectors = torch.empty(self.layer_p, n_ctx, ctx_dim, dtype=dtype)
        # nn.init.normal_(ctx_vectors, std=0.02)
        # self.ctx = nn.Parameter(ctx_vectors)  # to be optimized
        self.ctx = nn.ParameterList([nn.Parameter(torch.empty(n_ctx, 512, dtype=dtype)) for _ in range(self.layer_p - 1)])
        for single_para in self.ctx:
            nn.init.normal_(single_para, std=0.02)
    ...
    def forward(self, ctx = None):
        ...
        if ctx0.dim() == 2:
            ctx0 = ctx0.unsqueeze(0).expand(self.n_cls, -1, -1)

        new_ctx = []
        for i in range(len(ctx)):
            new_ctx.append(ctx[i].unsqueeze(0).expand(self.n_cls, -1, -1))
            
        prefix = self.token_prefix
        suffix = self.token_suffix
        prompts = torch.cat([prefix, ctx0, suffix], dim=1)
        return prompts, new_ctx
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.text_encoder = TextEncoder(cfg, clip_model, self.n_cls)
    ...
    def forward(self, image, label_idx = None, ctx = None, vis_ctx = None):
        ...
        if label_idx != None:
            ...
            new_ctx_t = []
            for c in ctx_t:
                new_ctx_t.append(c[label_idx])
            text_features = self.text_encoder(prompts.half(), tokenized_prompts, new_ctx_t)
        else:
            text_features = self.text_encoder(prompts.half(), tokenized_prompts, ctx_t)
        ...
        if self.training:
            return logits_ce, reg_image*10, reg_text*25
        else:
            return logits_ce
...
class MAMLFewShotClassifier(nn.Module):
    def __init__(self, cfg, model, classnames, device, layers, meta_step):
        ...
        #     nn.Linear(input_dim, input_dim)
        # ).to(device=self.device)
    def update_params(self, ctx, grads_ce, grads_reg, lr, textual=True):
        ...
        for i in range(self.layers):
            if textual:
                grads_per_layer_0 = self.regularizer_t[i](grads_ce[i].float()).half()
                grads_per_layer_1 = self.regularizer_t[i](grads_reg[i].float()).half()
            else:
                grads_per_layer_0 = self.regularizer_i[i](grads_ce[i].float()).half()
                grads_per_layer_1 = self.regularizer_i[i](grads_reg[i].float()).half()
    ...
    def forward(self, image, label, lr):
        ...
        for i in range(N):
            ...
            task_losses = []
            ctx, vis_ctx = [self.model.prompt_learner.ctx0.clone()], []
            for c in self.model.prompt_learner.ctx:
                ctx.append(c.clone())
            for v in self.model.vision_prompt_learner.ctx:
                vis_ctx.append(v.clone())
            
            for k in range(self.meta_step):

                logits_ce_sup, reg_image, reg_text = self.model(image, cls, ctx, vis_ctx)
                support_loss = F.cross_entropy(logits_ce_sup, label)
                # loss_text = F.cross_entropy(logits_t_sup, label)
                # reg_image = F.cross_entropy(logits_i, label)

                grads_ce_wrt_ctx = autograd.grad(support_loss, ctx, retain_graph=True)
                # grads_t_wrt_ctx = autograd.grad(loss_text, ctx, retain_graph=True)[0]
                reg_t_wrt_ctx = autograd.grad(reg_text, ctx, retain_graph=True)
                if (len(grads_ce_wrt_ctx) == 1):
                    grads_ce_wrt_ctx = grads_ce_wrt_ctx[0]
                if (len(reg_t_wrt_ctx) == 1):
                    reg_t_wrt_ctx = reg_t_wrt_ctx[0]
                ...
                new_ctx = self.update_params(ctx=ctx, grads_ce=grads_ce_wrt_ctx, grads_reg=reg_t_wrt_ctx, lr=lr, textual=True)

                grads_ce_wrt_vis = autograd.grad(support_loss, vis_ctx, retain_graph=True)
                # grads_i_wrt_vis = autograd.grad(loss_image, vis_ctx, retain_graph=True)[0]
                reg_i_wrt_vis = autograd.grad(reg_image, vis_ctx, retain_graph=True)
                if (len(grads_ce_wrt_vis) == 1):
                    grads_ce_wrt_vis = grads_ce_wrt_vis[0]
                if (len(reg_i_wrt_vis) == 1):
                    reg_i_wrt_vis = reg_i_wrt_vis[0]
                ...
                new_vis_ctx = self.update_params(ctx=vis_ctx, grads_ce=grads_ce_wrt_vis, grads_reg=reg_i_wrt_vis, lr=lr, textual=False)

                if k == self.meta_step - 1:
                    logits_ce_query, _, _ = self.model(image_li[i], cls_li[i], new_ctx, new_vis_ctx)
