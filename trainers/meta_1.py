...
class  LSLRGradientDescentLearningRule(nn.Module):
    ...
    def __init__(self, device, total_num_inner_loop_steps, layers, use_learnable_learning_rates=True, use_learnable_weight_decay=True, init_learning_rate=2e-3, init_weight_decay=1.0):
        ...
        self.init_learning_rate.to(device = device)
        ...
        self.init_weight_decay.to(device=device)
        self.layers = layers
        ...
        for i in range(layers):
            ...
            self.weight_list.append(nn.Parameter(
                data=torch.ones(self.total_num_inner_loop_steps) * self.init_weight_decay,
    ...
    def update_params_t(self, ctx, support_loss_mix, support_loss_t, num_step):
        ...
        for i in range(self.layers):
             per_step_task_embedding.append(ctx[i,:,:])
        for i in range(self.layers):
            per_step_task_embedding.append(support_grads_mix[i,:,:])
        for i in range(self.layers):
            per_step_task_embedding.append(support_grads_t[i,:,:])
        per_step_task_embedding = torch.stack(per_step_task_embedding)
        generated_params = self.regularizer_t(per_step_task_embedding)
        generate_weight, generated_alpha, generated_beta = torch.split(generated_params, split_size_or_sections=self.layers)

        new_ctx = [] 
        for i in range(self.layers):
            new_ctx.append(generate_weight[i,:,:] * self.weight_list[i][num_step] * ctx[i,:,:] - generated_alpha[i,:,:] * self.alpha_list[i][num_step] * grads_wrt_params_a[i,:,:] \
                           - generated_beta[i,:,:] * self.beta_list[i][num_step] * grads_wrt_params_b[i,:,:])
    ...
    def update_params_i(self, vis_ctx, support_loss_mix, support_loss_i, num_step):
        ...
        per_step_task_embedding = []
        for i in range(self.layers):
             per_step_task_embedding.append(vis_ctx[i,:,:])
        for i in range(self.layers):
            per_step_task_embedding.append(support_grads_mix[i,:,:])
        for i in range(self.layers):
            per_step_task_embedding.append(support_grads_i[i,:,:])
        per_step_task_embedding = torch.stack(per_step_task_embedding)
        ...
        new_vis_ctx = [] 
        for i in range(self.layers):
            new_vis_ctx.append(generate_weight[i,:,:] * self.weight_list[i][num_step] *  vis_ctx[i,:,:] - generated_beta[i,:,:] * self.alpha_list[i][num_step] * grads_wrt_params_b[i,:,:] - \
