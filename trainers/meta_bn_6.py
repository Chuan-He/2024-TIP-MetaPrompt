...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label_idx = None, ctx = None, vis_ctx = None, phase = None):
        ...
        if self.training:
            ...
            if phase == 1:
                return logits_ce, logits_i, reg_image
            elif phase == 2:
...
class MAMLFewShotClassifier(nn.Module):
    def __init__(self, cfg, model, classnames, device, layers, steps, init_lr):
        ...
        ctx_dim = 512
        self.regularizer_t = nn.Sequential(nn.Linear(ctx_dim*2, ctx_dim, bias=False),nn.ReLU(inplace=True),nn.Linear(ctx_dim, ctx_dim, bias=False)).to(device=self.device)
        #self.regularizer_t = nn.Linear(ctx_dim, ctx_dim, bias=False).to(device=self.device)
        #self.regularizer_t = nn.Sequential(nn.Linear(ctx_dim*3, ctx_dim, bias=False)).to(device=self.device)
        ...
        self.regularizer_i = nn.Sequential(nn.Linear(vis_dim*2, vis_dim, bias=False),nn.ReLU(inplace=True),nn.Linear(vis_dim, vis_dim, bias=False)).to(device=self.device)
        #self.regularizer_i = nn.Sequential(nn.Linear(vis_dim*3, vis_dim, bias=False)).to(device=self.device)
    ...
    def update_params(self, ctx, per_step_task_embedding, lr):
        ...
        """
        grads_0, grads_1 = per_step_task_embedding
        grads_all = torch.cat([grads_0, grads_1], dim=-1)

        if ctx.shape[-1] == 512:
            reg_grads_all = self.regularizer_t(grads_all.float()).half()

        else:
            reg_grads_all = self.regularizer_i(grads_all.float()).half()
        
        new_ctx = ctx - lr * (grads_0 + reg_grads_all)

        return new_ctx
    
    def forward(self, image, label, lr, phase):
        total_losses = []
        if phase == 0:                
            ...
            for i in range(N):
                ...
                for s in range(self.steps):
                    ...
                    logits_ce_sup, logits_t, reg_text= self.model(image=image, label_idx=cls, ctx=txt_ctx, vis_ctx=None, phase=0)
                    ...
                    grads_wrt_ctx_1 = autograd.grad(reg_text, txt_ctx, retain_graph=True)[0]
                    per_step_task_embedding = [grads_wrt_ctx_0, grads_wrt_ctx_1]
                    txt_ctx = self.update_params(ctx=txt_ctx, per_step_task_embedding=per_step_task_embedding, lr=lr)

                    if s == self.steps - 1:
                        logits_ce_query, _, _ = self.model(image=image_li[i], label_idx=cls_li[i], ctx=txt_ctx, vis_ctx=None, phase=0)
            ...
            for i in range(1):
                ...
                vis_ctx = self.model.vision_prompt_learner.ctx.clone()
                logits_ce_sup, logits_i, reg_image = self.model(image=image, label_idx=None, ctx=None, vis_ctx=vis_ctx, phase=1)

                support_loss = F.cross_entropy(logits_ce_sup, label)
                loss_i = F.cross_entropy(logits_i, label)
                grads_wrt_vis_0 = autograd.grad(support_loss, vis_ctx, retain_graph=True)[0]
                #rads_wrt_vis_1 = autograd.grad(loss_i, vis_ctx, retain_graph=True)[0]
                ...
                vis_ctx = self.update_params(ctx=vis_ctx, per_step_task_embedding=per_step_task_embedding, lr=lr)

                logits_ce_query, _, _= self.model(image=image, label_idx=None, ctx=None, vis_ctx=vis_ctx, phase=1)
...
class Meta_B2N(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        meta_loss = self.meta_model(image, label, lr, 0)
        ...
        lr=self.get_current_lr()
        meta_loss = self.meta_model(image, label, lr, 1)
        ...
        # meta_loss = self.meta_model(image, label, 0)
