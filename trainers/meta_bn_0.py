...
class  LSLRGradientDescentLearningRule(nn.Module):
    ...
    def update_params(self, ctx, grads_wrt_params_a, grads_wrt_params_b, generated_alpha, generated_beta, lr, num_step):
        ...
        new_ctx = [] 
        for i in range(self.layers):
            new_ctx.append(ctx[i,:,:] - generated_alpha[i] * (lr) * grads_wrt_params_a[i,:,:] \
                            - generated_beta[i] *  (lr) * grads_wrt_params_b[i,:,:])
    ...
    def forward(self, image, label, lr):
        ...
        for i in range(N):
            ...
            for k in range(self.meta_step):
                ...
                logits_ce_sup, logits_i_sup, logits_t_sup, reg_text, reg_image = self.model(image, cls, ctx, vis_ctx)
                support_loss = F.cross_entropy(logits_ce_sup, label)
                loss_text = F.cross_entropy(logits_t_sup, label) + reg_text
                loss_image = F.cross_entropy(logits_i_sup, label) + reg_image

                grads_wrt_ctx = autograd.grad(support_loss, ctx, retain_graph=True)[0]
                reg_wrt_ctx = autograd.grad(loss_text, ctx, retain_graph=True)[0]
                ...
                reg_wrt_vis = autograd.grad(loss_image, vis_ctx, retain_graph=True)[0]
                ...
                if k == self.meta_step - 1:
                    logits_ce_query, _, _, _, _ = self.model(image_li[i], cls_li[i], ctx, vis_ctx)
...
class Meta_B2N(TrainerX):
    ...
    def build_model(self):
        ...
        self.meta_model = MAMLFewShotClassifier(cfg, self.model, classnames, self.device, self.num_layers, self.meta_step, self.meta_lr)
    ...
    def forward_backward(self, batch):
        ...
        logits_ce, _, _, _, _ = self.model(image)
        ...
        meta_loss = self.meta_model(image, label, lr=self.meta_sched.get_lr()[0])
