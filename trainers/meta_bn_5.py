...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label_idx = None, ctx = None):
...
class MAMLFewShotClassifier(nn.Module):
    ...
    def update_params(self, ctx, per_step_task_embedding, k):
        ...
        for i in range(self.layers):
            ...
            new_ctx.append(ctx[i,:,:] - self.lr[k][i] * reg_grads_0 - self.lr[k][i] * reg_grads_1)
    ...
    def forward(self, image, label):
        ...
        for i in range(N):
            ...
            for k in range(self.steps):
                ...
                logits_ce_sup, reg_text = self.model(image, cls, ctx=ctx)
                ...
                ctx = self.update_params(ctx=ctx, per_step_task_embedding=per_step_task_embedding, k=k)

                if k == self.steps - 1:
                    logits_ce_query, _ = self.model(image_li[i], cls_li[i], ctx=ctx)
...
class Meta_B2N(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        loss = F.cross_entropy(logits, label) + reg_image
